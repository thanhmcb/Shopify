function handleDropDownFilters(item) { }

document.addEventListener("DOMContentLoaded", (event) => {
  document.querySelectorAll("[data-filter-item]").forEach((item) => {
    handleDropDownFilters(item);
  });
});
...
var __toESM = (mod, isNodeMode, target) => (
  ...
  __copyProps(
    ...
    // compatible transform (i.e. "__esModule" haheader-sidebar__main-panels not been set), then set
...
initialize_fn = function () {
  ...
  animate(
    __privateGet(this, _scroller, scroller_get),
    {
      transform: [
        "translateX(0)",
        `translateX(calc(var(--transform-logical-flip) * ${__privateGet(this, _direction, direction_get) * 100
        }%))`,
      ],
    },
    {
      ...
      case "open":
        ...
        if (
          ...
          animationControls.finished.then(() => {
            ...
            //    document.querySelector(".cart--recommendations").style.left =
            //      "-240px";
          });
          ...
          //  document.querySelector(".cart--recommendations").style.left = "0px";
  ...
  createEnterAnimationControls() {
    ...
    return timeline3([
      ...
      [
        ...
        {
          transform: [
            `translateX(calc(var(--transform-logical-flip) * ${this.openFrom === "right" ? "100%" : "-100%"
            }))`,
            "translateX(0)",
          ],
        },
...
document.addEventListener("facet:update", async (event) => {
  ...
  const sectionId = document
    .querySelector(".shopify-section--main-collection")
  ...
  const split = event.detail.url.search.split("section_id=");
  const url =
    ...
    "section_id=" +
    sectionId;
  const shopifySection = document.getElementById(
  ...
  options.forEach((value, index) => {
    let input = this.optionSelectors[index].querySelector(
      `input[type="radio"][name="option${index + 1}"][value="${CSS.escape(
        value
      )}"], input[type="hidden"][name="option${index + 1
...
doPredictiveSearch_fn = async function () {
  ...
  const url = `${window.Shopify.routes.root}search${this.supportsPredictiveApi() ? "/suggest" : ""
    ...
    queryParams = `q=${__privateGet(this, _queryInput).value
...
onBeforeShow_fn = async function () {
  let event = await fetchCart;

  /*let productsInCart = event.items.map((item) => {
    return item.product_id;
  });*/

  //renderCartRecommendations(productsInCart);

  //  const recom = this.querySelector(".cart--recommendations");
  /*  animate14(
      recom,
      { opacity: [0, 1] },
      { delay: 0.5, duration: 0.25, easing: [0.25, 0.46, 0.45, 0.94] }
    );*/
  /* animate14(
     recom,
     { transform: ["translateX(300px)", "translateX(0)"] },
     { delay: 0.75, duration: 0.25, easing: [0.25, 0.46, 0.45, 0.94] }
   );*/
  ...
  if (checkOutButton) {
    checkOutButton.addEventListener("click", (e) => {
      if (!termsCheckbox.checked) {
        alert("Du skal acceptere vores vilkår og betingelser for at fortsætte.");
        e.preventDefault();
        return false;
      }
    });
...
var CountdownTimerFlip = class extends HTMLElement {
  constructor() {
    ...
    let flipHtml = [...this.textContent].map(
      () =>
        `<countdown-timer-flip-digit part="digit" ${this.hasAttribute("animate") ? "animate" : ""
...
openCollapsiblePanel_fn = function (event) {
  ...
  if (matchesMediaQuery("md-max")) {
    animate18(
      this.querySelector(".header-sidebar__main-panel"),
      { opacity: [1, 1], transform: ["translateX(0)", "translateX(-10px)"] },
      { duration: 0.25 }
    );
  }
};
...
switchPanel_fn = async function (fromPanel, toPanel) {
  ...
  if (fromPanel) {
    await animate18(fromPanel, { opacity: [1, 1] }, { duration: 0 })
      .finished;
    fromPanel.hidden = true;
  ...
  timeline9([
    ...
    [
      ...
      { opacity: [1, 1], transform: ["translateX(8px)", "translateX(0)"] },
      { duration: 0.15, at: "0", delay: 0 },
    ],
    [
...
updateButtonLink_fn = function (event) {
  ...
  if (productCard.hasAttribute("handle")) {
    this.nextElementSibling.href = `${Shopify.routes.root
...
/*
async function fetchHTML(product) {
  const parser = new DOMParser();

  const result = await fetch(`/products/${product}?section_id=bogo-cart-slider`)
...
async function loadBOGO() {
  if (document.querySelector("#bogoJSON")) {
    const productHandles = JSON.parse(
      document.querySelector("#bogoJSON").innerText
    );

    const asyncRes = await Promise.all(
      productHandles.map(async (product) => {
        let response = await fetchHTML(product.handle);

        let HTML = `
      <div class="Grid__Cell 1/1--phone 1/2--tablet 1/1--lap-and-up">
      <h2 class="SectionHeader__Heading Heading u-h4">${product.headline}</h2>
      ${response.querySelector(".grab-inner").innerHTML}
      </div>`;
    ...
    asyncRes.map((html) => {
      document.querySelector(".BOGO--grid").innerHTML += html;
    });
  }
}

async function fetchRecommendations(product, limit) {
  ...
  result = await fetch(
    ...
    `recommendations/products.json?product_id=${product}&limit=${limit}&intent=complementary`
  )
  ...
  if (productIds.length < limit) {
    result = await fetch(
      window.Shopify.routes.root +
      `recommendations/products.json?product_id=${product}&limit=${limit}&intent=related`
    )
      .then((response) => response.json())
      .then(({ products }) => {
        productIds = [...productIds, ...products];
...
async function renderCartRecommendations(productsInCart) {
  let array = [];
  let filter = [];

  //const productsInCart = JSON.parse(document.querySelector('#productsInCart').value)
  let limit = Math.ceil(6 / productsInCart.length);

  await Promise.all(
    productsInCart.map(async (product) => {
      let response = await fetchRecommendations(product, 6);

      //let firstElements = response.slice(0, limit)
      // let test = array.filter((v, i, a) => a.findIndex(v2 => (v2.id === v.id)) === i)
  ...
  filter = array.filter((v, i, a) => a.findIndex((v2) => v2.id === v.id) === i);
  let elements = filter.slice(0, 6);

  const productHTML = await Promise.all(
    elements.map(async (product) => {
      let response = await fetchHTML(product.handle);
  ...
  /* document.querySelector(".cart--recommendations__container").innerHTML = "";
   productHTML.map((html, index) => {
     if (index < 6) {
       document.querySelector(".cart--recommendations__container").innerHTML +=
         html;
     }
...
async function renderProductRecommendations(product, limit) {
  const productRecommendations = await fetchRecommendations(product, limit);
  ...
  productHTML.map((html) => {
    document.querySelector(".BOGO--carousel").innerHTML += html;
  });
}

document.addEventListener("variant:add", function (event) {
  console.log(event);
  let productsInCart = event.detail.cart.items.map((item) => {
    return item.product_id;
  });
...
/*
const cartDrawerClose = document.querySelector('#cart-drawer [is="dialog-close-button"]');
const cartRecommendations = document.querySelector('.cart--recommendations')


cartDrawerClose.addEventListener('click', () => {
  cartRecommendations.style.left = '0px';
})
