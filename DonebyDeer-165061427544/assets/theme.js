    ...
    async _onSubmit(event) {
      ...
      this.dispatchEvent(new CustomEvent("cart-notification:show", {
        ...
        detail: {
          ...
          ? responseJson["description"].replace("Required parameter missing or invalid: id", "Please pick a size")
...
// js/custom-element/section/cart/free-shipping-bar.js
 var FreeShippingBar = class extends HTMLElement {
  ...
  _onCartUpdated(event) {
    ...
    console.log(totalPrice, this.threshold, parseFloat(totalPrice) / this.threshold);
    this.style.setProperty("--progress", Math.min(parseFloat(totalPrice) / this.threshold, 1));
  }
};
...
window.customElements.define("free-shipping-bar", FreeShippingBar);
 ...
 // js/custom-element/section/cart/free-shipping-bar-custom.js
  var FreeShippingBarCustom = class extends HTMLElement {
    connectedCallback() {
      document.documentElement.addEventListener("cart:updated", this._onCartUpdated.bind(this));
      var itemGiftcard = $('#mini-cart-form .gift-card-item').length;
      var itemProduct = $('#mini-cart-form .physical-item').length;
      if (itemGiftcard >= 1 && itemProduct === 0) {
        $('.only-gift-cart-shipping').css('display', 'block');          
      }else if (itemGiftcard === 0 && itemProduct >= 1){
        $('.mix-shipping').css('display', 'block');
      }
      else if (itemGiftcard >= 1 && itemProduct >= 1){
        $('.mix-shipping').css('display', 'block');
        $('.mix-text').css('display', 'block');
      }
    }
    ...
    _onCartUpdated(event) {
      ...
      if (itemGiftcard >= 1 && itemProduct === 0) {
        $('.only-gift-cart-shipping').css('display', 'block');          
        $('.not-gift-cart-shipping').css('display', 'none');
      }else if (itemGiftcard === 0 && itemProduct >= 1){
        $('.mix-shipping').css('display', 'block');
      }
      else if (itemGiftcard >= 1 && itemProduct >= 1){
  ...
  window.customElements.define("free-shipping-bar-custom", FreeShippingBarCustom);



  // js/custom-element/section/cart/shipping-price.js
  var ShippingPrice = class extends HTMLElement {
    connectedCallback() {
      document.documentElement.addEventListener("cart:updated", this._onCartUpdated.bind(this));
      var itemGiftcard = $('#mini-cart-form .gift-card-item').length;
      var itemProduct = $('#mini-cart-form .physical-item').length;
      var itemFreeshiping = $('.drawer__content .free-shipping').length;
      if (itemGiftcard >= 1 && itemProduct === 0 || itemFreeshiping >=1 ) {
        $('.mini-cart__actions--shipping-price-text').css('display', 'none');             
      }else if (itemGiftcard === 0 && itemProduct >= 1){
        $('.mini-cart__actions--shipping-price-text').css('display', 'block');
      }
      else if (itemGiftcard >= 1 && itemProduct >= 1){
        $('.mini-cart__actions--shipping-price-text').css('display', 'block');
      }
  ...
  window.customElements.define("shipping-price", ShippingPrice);
