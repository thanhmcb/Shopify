  ...
  },
  {
    ...
    events: {
      "click a[data-wk-share]": (event) => {
        ...
        if (!WishlistKing.toolkit.config.session.customer) {
          window.location = [
            ...
            {% unless limit and item_count > limit %}
              ...
              <div class="wk-grid__item {% if onsale %}wk-product--sale{% endif %}" data-wk-item="{{ product.wishlist_item_id }}">
                ...
                <div class="wk-product-info">
                  ...
                  <span class="wk-product-price--current">{{ variant.price | money: false, false }}</span>
                    <span class="wk-product-price--compare">{{ variant.compare_at_price | money: false, false }}</span>
                  </div>
                </div>
        ...
        WishlistKing.toolkit.getItem(itemId).then((product) => {
          WishlistKing.toolkit.initProductForm(form, product, {
            ...
            onFormSubmit: async (event) => {
              ...
              const submitButton = form.querySelector("[data-wk-add-to-cart]");
      ...
      <form class="wk-product-form" action="/cart/add" method="post">
        ...
        {% if current_variant.available %}
          ...
          {% if product_type == "bundle" %}
            <a class="wk-product-form__submit" href="{{ product | variant_url }}">
              {{ locale.view_product }}
            </a>
          {% else %}
            <button type="submit" class="wk-product-form__submit" data-wk-add-to-cart="{{ product.wishlist_item_id }}">
              {{ locale.add_to_cart }}
            </button>
          {% endif %}
        {% else %}
          {% if product.metafields.my_fields.c5availability contains "coming soon" %}
          {% assign label = locale.product_coming_soon_custom %}
          {% elsif product.metafields.my_fields.c5availability contains "available in stores" %}
          {% assign label = locale.product_in_stores_custom %}
          {% else %}
          {% assign label = locale.product_soldout_custom %}
          {% endif %}
          <a class="wk-product-form__submit" href="{{ product | variant_url }}">
            {{ label }}
          </a>
